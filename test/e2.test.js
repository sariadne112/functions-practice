...
import { f } from "file:///home/navaari/Devslopes projects/functions-practice/exercises/e2.js";
...
describe("totalRoomsArea", () => {
  ...
  it("Should return a correct value for diferent values", () => {
    const roomOneFirst = f.rectArea(
      rooms.room1.first[0],
      rooms.room1.first[1]
    );
    const roomTwoFirst = f.rectArea(
      rooms.room2.first[0],
      rooms.room2.first[1]
    );
    const roomThreeFirst = f.rectArea(
      rooms.room3.first[0],
      rooms.room3.first[1]
    );
    const roomOneSecond = f.rectArea(
      rooms.room1.second[0],
      rooms.room1.second[1]
    );
    const roomTwoSecond = f.rectArea(
      rooms.room2.second[0],
      rooms.room2.second[1]
    );
    const roomThreeSecond = f.rectArea(
      rooms.room3.second[0],
      rooms.room3.second[1]
    );
    expect(
      f.totalRoomsArea(roomOneFirst, roomTwoFirst, roomThreeFirst)
    ).toEqual(160);
    expect(
      f.totalRoomsArea(roomOneSecond, roomTwoSecond, roomThreeSecond)
    ).toEqual(148);
  });
});
