...
import { f } from "file:///home/navaari/Devslopes projects/functions-practice/exercises/e1.js";
...
describe("logMessageWithMyName", () => {
  ...
  it("Must use NAMED function syntax", () => {
    expect(
      f.logMessageWithMyName.hasOwnProperty("prototype")
    ).toEqual(true);
  });
});
...
describe("logMyInfo", () => {
  ...
  it("The function logs a message using two arguments passed", () => {
    ...
    expect(logSpy).toBeCalledWith(
      `${testStart + testNameOne + testMiddle + testAgeOne + testEnd}`
    );
    expect(logSpy).toHaveBeenLastCalledWith(
      `${testStart + testNameTwo + testMiddle + testAgeTwo + testEnd}`
    );
  });
